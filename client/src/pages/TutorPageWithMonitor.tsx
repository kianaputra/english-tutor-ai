...
export default function TutorPageWithMonitor() {
  ...
  // Face detection hook
  const { isInitialized: faceDetectionReady } = useFaceDetection({
    enabled: true,
    onFaceClose: () => {
  ...
  // Lip-sync hook
  const { currentMouthShape } = useLipSync({
    enabled: true,
    isSpeaking,
  });
  ...
  return (
    <MonitorFrame orientation={orientation}>
